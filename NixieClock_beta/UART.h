...
#define _TIME_OUT 2 //таймаут приёма 2мс

#define _DELAY_START _delay = 0; TIMSK2 |= (1 < OCIE2B)
#define _DELAY_STOP  TIMSK2 &= ~(1 < OCIE2B)
...
void dataChannelInit(uint32_t baudrate) //инициализация UART
{
  ...
  UCSR0A = (1 << U2X0); //устанавливаем удвоение скорости
...
ISR(USART_UDRE_vect) //прерывание пустого буфера передачи
{
  ...
  if (++_TRANSFER_BUFFER_START >= _TRANSFER_BUFFER_END) { //если весь пакет передан
    ...
    UCSR0B &= ~ (1 << UDRIE0); //выключаем прерывания передачи
  }
}
//----------------------------------Отправка команды----------------------------------
void sendCommand(uint8_t command) //отправка команды
{
  if (!_TRANSFER_BUFFER_END) { //отправляем только после опустошения буфера
    ...
    UCSR0B |= (1 << UDRIE0); //разрешаем прерывание по завершению передачи
  }
}
//-------------------------Отправка информации----------------------------------------------------
