...
Порядок записи нового загрузчика(потребуется isp программатор и программа avrdude_prog):
    ...
    Ждём окончания загрузки(появится надпись "Загрузка прошивки завершена").
...
Маска дешефратора и порядок катодов указываются в "config.h".
    ...
    Д![Connecting peripherals](https://github.com/user-attachments/assets/2c59d1a0-7aa2-4eb3-bb85-82e60dbbe5a0)
ля запуска меню отладки - необходимо зажать левую кнопку и подать питание на часы.
