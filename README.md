...
<img width="3225" height="2402" alt="Connecting peripherals" src="https://github.com/user-attachments/assets/83a54a0e-688a-4ef2-b21c-7cddc4fbc8cc" /><br>
    ...
    При использовании модуля ESP8266 подтяжку шины I2C желательно организовывать от источника 3.3в. Подтяжка шины должна быть строго только в одном месте!
    ...
    Для формирования нового языкового пакета веб интерфесйса необходимо создать свой пакет перевода во вкладке "languages.h".
...
Для синхронизации хода секунд желательно подключить пин SQW модуля DS3231 к пину ардуино D2..3, если у вас модуль DS3231mini, то необходимо найти 3-ю ножку микросхемы и также проводком подпаять к пину D2..3.

Для настройки прошивки под другую конфигурацию плат, отличающуюся от преднастроенных, необходимо указать BOARD_TYPE 0, пины подключения периферии в connection.h и настроить используемую периферию в userConfig.h, config.h.<br>
Маска дешефратора и порядок катодов указываются в config.h. Свои настройки можно разместить в boards.h для более удобного переноса на новые версии прошивки.

Так-же не забудьте выбрать свою плату в "userConfig.h" - BOARD_TYPE.<br>
Для кастомной платы используйте - BOARD_TYPE 0.<br>
...
<img width="1850" height="1578" alt="Wireless sensor" src="https://github.com/user-attachments/assets/1bd39468-c2f3-4bb0-981c-aec3b4b6388e" /><br>
...
Настройка и подключение беспроводного датчика температуры:
    ...
    После успешного перехода датчика в режим настройки светодиод индикации состояния будет светится постоянно, затем появится точка доступа "WirelessSensor"(пароль по умолчанию 1234567890).
    До отключения точки доступа у вас будет 90 сек, необходимо подключится к точке доступа "WirelessSensor" и открыть веб интерфейс по ссылке - http://192.168.4.4/
    ...
    После подключения датчика к точке доступа в веб интерфейсе часов появится уведомление с предложением подключить новый датчик температуры, после нажатия на кнопку "Да" датчик будет успешно привязан к часам(если уведомление не появляется, необходимо повторно ввести беспроводной датчик в режим настройки).
После настройки беспроводной датчик будет автоматически отправлять показания с заданным интервалом.
Посмотреть информацию об уровне сигнала/заряде батареи можно во вкладке "Об устройстве", данные датчика температуры и интервал во вкладке "Микроклимат".
<br><br>

Настройка секундных точек:

    NEON_DOT 0 - Обычная светодиодная секундная точка(пин подключения указывается в SECL_PIN)(возможно комбинирование с точками в индикаторах)
    NEON_DOT 1 - Одна неоновая лампа в качестве секундной точки(пин подключения указывается в SECL_PIN)(возможно комбинирование с точками в индикаторах)
    NEON_DOT 2 - Две неоновые лампы в качестве секундных точек(пины подключения указываются в SECL_PIN и SECR_PIN)(возможно комбинирование с точками в индикаторах)
    NEON_DOT 3 - Точки в индикаторах в качестве секундных точек(пины подключения указываются в DOTSL_PIN и DOTSR_PIN)
    NEON_DOT 4 - Декатрон в качестве секундной точки(пины подключения указываются в DECATRON_K0_PIN, DECATRON_K1_PIN, DECATRON_PK1_PIN и DECATRON_PK2_PIN)(возможно комбинирование с точками в индикаторах)

Пины подключения периферии к сдвиговому регистру указываются в connection.h: ANODE_1_PIN..ANODE_6_PIN, SECL_PIN, SECR_PIN, DOTSL_PIN, DOTSR_PIN. Доступные номера пинов 0..7.
...
Динамическая индикация на x2 К155ИД1:
...
При использовании сдвигового регистра есть ограничения на пины D13-D10, а именно:
    ...
    D10 может быть использован для остальной периферии в качестве выхода(ВВ преобразователь, секундные точки, декатрон, подсветка, бузер, управление питанием усилителя, RX DF плеера при использовании софтового UART, MOSI/SCK/CS SD карты, защёлка сдвигового регистра).
<br>
...
Программно -
    ...
    10 режимов анимации смены минут(Есть случайный режим).
    3 режима анимации смены секунд для 6-ти ламп.
    3 режима подсветки для обычных диодов и 16 для адресных WS2812B.
    5 режимов анимации секунд для декатрона.
    5 режимов работы секундных точек + 2 для двух неоновых ламп и 7 для разделительных точек.
...
Настройка будильника:  Удерживать "левую" кнопку<br>
Настройка нескольких будильников "ALARM_TYPE 2":

В режиме часов удержание левой клавиши – переход к настройке будильников.<br>
...
Как добавить свои мелодии будильника:<br>
<br>
...
Пункты меню:
    ...
    01 - Бузер(Формат времени(12ч | 24ч))
         Плеер(Глюки(0 - выкл | 1 - вкл) : Формат времени(12ч | 24ч))
    02 - Бузер(Глюки(0 - выкл | 1 - вкл))
         Плеер(Голос озвучки(0 - Алёна | 1 - Филипп) : Громкость озвучки(0..15))
    03 - Бузер(Звук кнопок/секунд(0 - выкл | 1 - вкл звук кнопок | 2 - вкл звук кнопок и секунд) : Озвучка часа(0 - выкл | 1 - вкл)
         Плеер(Озвучка действий(0 - выкл | 1 - вкл) : Озвучка часа(Температура/Новый час))
    04 - Время беззвучного режима(Начало : Конец)(Выключить беззвучный режим: начало = конец)
...
Управление кнопками В РЕЖИМЕ ЧАСОВ:<br>
<br>
