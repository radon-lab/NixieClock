...
Что планируется:
    ...
    Добавить индивидуальное свечение адресных светодиодов для каждого меню.
    Добавить поддержку модуля ESP-01 для настройки через веб интерфейс.
    Добавить поддержку датчика движения RCWL-0516.
...
Программно:
    ...
    Ночной режим(Октлючение ламп через заданное время, когда ночная яркость индикаторов установлена в 0)
...
Пункты меню:
    ...
    11 - Выбор анимации антиотравления индикаторов(0 - перебор всех индикаторов | 1 - перебор одного индикатора | 2 - перебор одного индикатора с отображением времени) и анимации секунд(0 - выкл | 1 - плавная смена яркостью | 2 - перемотка по порядку числа | 3 - перемотка по порядку катодов в лампе)(антиотравление : анимация секунд)
...
Клик по "средней" кнопке отображает текущий режим подсветки.
    ...
    Клик по "средней" кнопке(для адресных светодиодов) - переключает режим подсветки(0 - выкл | 1 - статичная | 2 - дыхание | 3 - дыхание со сменой цвета при затухании | 4 - бегущий огонь | 5 - бегущий огонь со сменой цвета | 6 - волна | 7 - волна со сменой цвета | 8 - плавная смена цвета | 9 - радуга | 10 - конфетти)
