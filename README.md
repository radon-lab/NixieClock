...
Так-же не забудьте выбрать свою плату - в "userConfig" - BOARD_TYPE. Для других плат на 4 лампы используйте - BOARD_TYPE 3, на 6 ламп - BOARD_TYPE 4.
...
Что планируется:
    ...
    Добавить управление разделительными точками в индикаторах ИН12/14.
    Голосовое сопровождение на DFPlayer.
    
Что поддерживается:
Периферия:

    Датчики температуры DHTxx, DS18xx - по умолчанию подключаются к пину D1 с подтяжкой 4.7-10кОм к +5в.
    ...
    Дополнительная кнопка для управления таймером - секундомером - по умолчанию подключается к D0.
    Преобразователь ВВ напряжения(Затвор мосфета) - подключается к пину D9.
    Аналоговые основные кнопки - по умолчанию подключаются к A7.
...
Программно:
    ...
    9 режимов перелистывания цифр(Есть случайны режим).
    3 режима подсветки для обычных диодов и 11 для адресных.
