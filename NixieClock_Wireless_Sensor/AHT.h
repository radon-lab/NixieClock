...
void readTempAHT(void) //чтение температуры/влажности
{
  ...
  if (!typeAHT) { //если датчик не определен
    if (!twi_beginTransmission(AHT10_ADDR)) { //выходим если нет ответа
      ...
      while (1) { //ждем окончания калибровки
        ...
        if (twi_beginTransmission(AHT10_ADDR, 1)) return; //запрашиваем чтение данных
        if ((twi_read_byte(TWI_NACK) & 0x88) == 0x08) break;
        if (((uint16_t)millis() - timer) >= AHT10_CALIBRATE_TIME) return; //выходим если таймаут
      }
    }
    else return;
  ...
  if (twi_beginTransmission(AHT10_ADDR)) return; //начало передачи
  ...
  while (1) { //ждем окончания замера
    ...
    if (((uint16_t)millis() - timer) >= AHT10_MEASURE_TIME) return; //выходим если таймаут
  }
