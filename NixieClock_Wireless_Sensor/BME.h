  ...
  switch (typeBME) { //чтение данных
    case BMP180_ADDR: {
        ...
        while (1) { //ожидаем окончания замера
          ...
          if (twi_requestFrom(BMP180_ADDR, BMP180_CONTROL_REG)) return; //запрашиваем чтение данных, если нет ответа выходим
          if (!(twi_read_byte(TWI_NACK) & 0x20)) break; //если замер завершён продолжаем
          if (((uint16_t)millis() - timer) >= BME_CHECK_TIMEOUT) return; //выходим если таймаут
        }

        if (twi_requestFrom(BMP180_ADDR, BMP180_DATA_OUT_REG)) return; //запрашиваем чтение данных, если нет ответа выходим
        ...
        if (sens.temp == 0x7FFF) sens.temp = (temp_raw + 8) >> 4; //установили температуру
        if (!sens.press) sens.press = (press_raw + ((temp_val_1 + temp_val_2 + 3791) >> 4)) * 0.00750062; //записываем давление в мм рт.ст.
      }
      break;
    case BME280_ADDR: {
        writeREG(BME280_ADDR, BME280_CONTROL_MEAS_REG, ((BME280_TEMP_OVERSAMP << 5) | (BME280_PRESS_OVERSAMP << 2) | BME280_MODE)); //устанавливаем разрешение датчика температуры и датчика давления, устанавливаем режим работы
        while (1) { //ожидаем окончания замера
          yield(); //обработка данных
          if (twi_requestFrom(BME280_ADDR, BME280_STATUS_REG)) return; //запрашиваем чтение данных, если нет ответа выходим
          if (!(twi_read_byte(TWI_NACK) & 0x08)) break; //если замер завершён продолжаем
          if (((uint16_t)millis() - timer) >= BME_CHECK_TIMEOUT) return; //выходим если таймаут
        }
        if (twi_requestFrom(BME280_ADDR, BME280_DATA_OUT_REG)) return; //запрашиваем чтение данных, если нет ответа выходим
        ...
        if (sens.temp == 0x7FFF) sens.temp = ((temp_raw * 5 + 128) >> 8) / 10; //установили температуру
        ...
        if (press_val_1) { //если значение не нулевое
          ...
          if (!sens.press) sens.press = (uint32_t)((int32_t)press_raw + ((press_val_1 + press_val_2 + CalibrationBME.PRESS_7) >> 4)) * 0.00750062; //записываем давление в мм рт.ст.
        }
        ...
        if (!sens.hum) sens.hum = (hum_val >> 12) / 1024.0; //записываем влажность в %
      }
      break;
  }
  
  settings.sensor |= (0x01 << SENS_BME); //установили флаг сенсора
}
