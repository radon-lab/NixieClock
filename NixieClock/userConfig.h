...
#define BACKL_MODE 0              //тип родсветки индикаторов(0 - хардверный шим(обычные светодиоды)(только D11)(только без SD плеера и хардверным UART) | 1 - софтверный шим(обычные светодиоды)(любой пин) | 2 - светодиоды WS2812B(любой пин))
...
#define NEON_DOT 0                //использовать неоновую лампу в качестве секундных точек(0 - точка светодиодная(только без SD карты) | 1 - точка неоновая лампа)
#define LAMP_NUM 4                //количесвто используемых ламп в часах(4 или 6)

#define PLAYER_MODE 0             //использовать DF плеер вместо бузера(0 - бузер | 1 - DF плеер | 2 - SD плеер)
#define UART_MODE 0               //режим работы UART DF плеера(0 - хардверный UART(только D1) | 1 - софтверный UART(любой пин)(только если адресные светодиоды или софтовый шим подсветки))

#define SENS_BME_DISABLE 0        //отключить поддержку датчиков температуры BMPxxx и BMExxx(0 - датчик используется | 1 - датчик не используется)
#define SENS_PORT_DISABLE 1       //отключить использование порта датчиков температуры DS18xx и DHTxx(0 - порт используется | 1 - порт не используется)
#define DOTS_PORT_DISABLE 1       //отключить использование порта разделительных точек в индикаторах(0 - порт используется | 1 - порт не используется)
#define AMP_PORT_DISABLE 1        //отключить использование порта управления питанием усилителя(0 - порт используется | 1 - порт не используется)
...
#define DEFAULT_KNOCK_SOUND 1      //звук нажатия клавиш по умолчанию(1 - звук включен | 0 - звук выключен)
#define DEFAULT_TEMP_SENSOR 0      //датчик температуры по умолчанию(0 - DS3231(встроенный) | 1 - BMP180/BMP085/BMP280/BME280 | 2 - DHT11/DHT12/MW33 | 3 - DHT21/DHT22(AM2301/AM2302) | 4 - DS18B20/DS18S20/DS1820)
