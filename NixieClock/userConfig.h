...
#define DS3231_ENABLE 1           //включить поддержку модуля часов DS3231(0 - модуль не используется | 1 - модуль используется)
...
#define SENS_AHT_ENABLE 0         //включить поддержку датчиков температуры AHT1x и AHT2x(0 - датчик не используется | 1 - используется датчик AHT10/AHT15/AHT20)
#define SENS_SHT_ENABLE 0         //включить поддержку датчиков температуры SHT2x и SHT3x(0 - датчик не используется | 1 - используется датчик SHT2x | 2 - используется датчик SHT3x | 3 - используются датчики SHT2x/SHT3x)
#define SENS_BME_ENABLE 0         //включить поддержку датчиков температуры BMPxxx и BMExxx(0 - датчик не используется | 1 - используется датчик BMP085/BMP180 | 2 - используется датчик BMP280/BME280 | 3 - используются датчики BMP085/BMP180/BMP280/BME280)
...
#define DEFAULT_TEMP_CORRECT 0     //коррекция температуры по умолчанию(-127..127)(0.1°c)(поддерживаются датчики - DS3231(встроенный), AHT10/AHT15/AHT20, SHT20/SHT21/SHT25/SHT30/SHT31/SHT35, BMP085/BMP180/BMP280/BME280, DHT11/DHT12/MW33, DHT21/DHT22(AM2301/AM2302), DS18B20/DS18S20/DS1820)
...
//NEON_DOT 0..1 - (0 - выкл | 1 - статичная | 2 - динамичная(плавно мигает)), (для точек в индикаторах)(3 - мигающая | 4 - бегущая | 5 - змейка | 6 - резинка | 7 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 8 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки))
//NEON_DOT 2 - (0 - выкл | 1 - статичная | 2 - динамичная(плавно мигает) | 3 - маятник), (для точек в индикаторах)(4 - мигающая | 5 - бегущая | 6 - змейка | 7 - резинка | 8 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 9 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки))
//NEON_DOT 3 - (0 - выкл | 1 - статичная | 2 - мигающая | 3 - бегущая | 4 - змейка | 5 - резинка | 6 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 7 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки))
...
#define DOT_OTHER_BLINK_TIME 0     //время включения секундной точки для дополнительной анимации "одиночное мигание"(100..500)(0 - один раз в секунду)(мс)
#define DOT_OTHER_DOOBLE_TIME 150  //время включения секундной точки для дополнительной анимации "двойное мигание"(100..1000)(мс)
...
#define AUTO_SHOW_MODES 4, 5, 6    //выбранные режимы отображения данных автоматического показа(указывать через запятую)(0 - пустая ячейка | 1 - дата | 2 - год | 3 - дата и год | 4 - температура | 5 - влажность | 6 - давление | 7 - температура и влажность)(не более 5 пресетов!)
#define AUTO_SHOW_TIMES 5, 5, 5    //задержки выбранных режимов отображения данных автоматического показа(указывать через запятую)(1..5)(сек)(не более 5 пресетов!)
...
#define SHOW_TEMP_MODE 0           //режим отображения температуры (только для esp)(0 - отображается температура заданная в веб интерфейсе(режим один датчик) | 1 - отображается температура заданная в веб интерфейсе(режим два датчика) | 2 - отображается температура с датчика установленного в часах(режим два датчика))
...
#define ALARM_TIMEOUT_SOUND 1      //таймаут до автоматического включения ожидания будильника по умолчанию(0..240)(0 - выкл повторное включение)(мин)
#define ALARM_TIMEOUT 15           //таймаут до полного отключения будильника по умолчанию(1..240)(м)
#define ALARM_WAIT 4               //ожидание будильника для повторного включения по умолчанию(0..240)(0 - выкл ожидание по короткому нажатию)(мин)

#define ALARM_AUTO_VOL_MIN 10      //минимальная громкость будильника в авто режиме(0..100)(%)
#define ALARM_AUTO_VOL_MAX 100     //максимальная громкость будильника в авто режиме(0..100)(%)
...
#define ALARM_ON_BLINK_DOT 1       //режим мигания точек если хоть один будильник включен(0..12)(описание ниже в таблице)
#define ALARM_WAIT_BLINK_DOT 2     //режим мигания точек если ожидаем повторного сигнала(0..12)(описание ниже в таблице)

//Режимы секундных точек
//NEON_DOT 0..1 - (0 - без реакции | 1 - мигает один раз в секунду | 2 - мигает два раза в секунду | 1 - выкл | 2 - статичная | 3 - динамичная(плавно мигает)), (для точек в индикаторах)(4 - мигающая | 5 - бегущая | 6 - змейка | 7 - резинка | 8 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 9 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки))
//NEON_DOT 2 - (0 - без реакции | 1 - мигает один раз в секунду | 2 - мигает два раза в секунду | 3 - выкл | 4 - статичная | 5 - динамичная(плавно мигает) | 6 - маятник), (для точек в индикаторах)(7 - мигающая | 8 - бегущая | 9 - змейка | 10 - резинка | 11 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 12 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки))
//NEON_DOT 3 - (0 - без реакции | 1 - мигает один раз в секунду | 2 - мигает два раза в секунду | 3 - выкл | 4 - статичная | 5 - мигающая | 6 - бегущая | 7 - змейка | 8 - резинка | 9 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 10 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки))
