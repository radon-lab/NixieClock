...
#define BACKL_TYPE 3              //тип подсветки индикаторов(0 - подсветка не используется | 1 - хардверный шим(обычные светодиоды)(только D11)(только без бузера, SD плеера и хардверным UART) | 2 - софтверный шим(обычные светодиоды)(любой пин)(только без IR приемника и декатрона) | 3 - светодиоды WS2812B(любой пин))
...
#define SHOW_DATE_BACKL_N 0        //цвет незначащих разрядов меню температуры(0..255)(только для светодиодов WS2812B)
...
#define DEFAULT_HOUR_SOUND_END 8   //время окончания звука смены часа по умолчанию(0..23)(ч)
...
//SECS_DOT 4 - (0 - без реакции | 1 - выкл | 2 - статичная | 3 - мигает один раз в секунду | 4 - мигает два раза в секунду | 5 - метроном | 6 - шажки | 7 - таймер | 8 - маятник), (для точек в индикаторах)(9 - мигающая | 10 - бегущая | 11 - змейка | 12 - резинка | 13 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 14 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки))
