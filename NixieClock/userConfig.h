...
#define SENS_AHT_ENABLE 0         //включить поддержку датчиков температуры AHT1x и AHT2x(0 - датчик не используется | 1 - используется датчик AHT10/AHT15 | 2 - используется датчик AHT20)
...
#define AUTO_SHOW_MODES 1, 2, 3    //выбранные режимы отображения данных автоматического показа(указывать через запятую)(0 - пустая ячейка | 1 - температура | 2 - влажность | 3 - давление | 4 - температура и влажность | 5 - дата | 6 - год | 7 - дата и год)(не более 5 пресетов!)
