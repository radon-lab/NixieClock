...
#define BOARD_TYPE 0              //тип платы часов(0 - кастомная плата(настройка маски в config.h) | 1 - [AlexGyver v1] ИН12 (только 4x ИН12) | 2 - [AlexGyver v1] ИН14 (6x ИН14 или 4x ИН14 + 2x ИН16) | 3 - [AlexGyver v2] ИН12 (индикаторы стоят правильно) | 4 - [AlexGyver v2] ИН12 turned (индикаторы перевернуты) | 5 - [AlexGyver v2] ИН14 (обычная и неоновая точка))
#define INDI_MODE 0               //режим индикации ламп(0 - динамический режим(x1 К155ИД1) | 1 - динамический режим(x2 К155ИД1))

#define BACKL_TYPE 0              //тип подсветки индикаторов(0 - подсветка не используется | 1 - хардверный шим(обычные светодиоды)(только D11)(только без бузера, SD плеера и хардверным UART) | 2 - софтверный шим(обычные светодиоды)(любой пин)(только без IR приемника и декатрона) | 3 - светодиоды WS2812B(любой пин))
#define SECS_DOT 0                //тип основных секундных точек(0 - точка светодиод | 1 - точка неоновая лампа(1 шт) | 2 - точка неоновая лампа(2 шт) | 3 - точка в индикаторе | 4 - точка в декатроне)
...
#define INDI_DOT_TYPE 0           //тип подключения секундных неоновых ламп при использовании сдвигового регистра(0 - прямое подключение к микроконтроллеру | 1 - подключение через сдвиговый регистр)(только для динамического режима(x1 К155ИД1))

#define INDI_SYMB_TYPE 0          //тип подключения анода символьного индикатора(0 - лампа не используется | 1 - прямое подключение к микроконтроллеру | 2 - подключение через сдвиговый регистр)(только для динамического режима(x1 К155ИД1))
...
#define DOTS_NUM 4                //количество ламп в которых используются разделительные точки(4 или 6)
...
//Настройки подсветки
#define BACKL_CONNECT_DIR 0       //направление подключения светодиодов WS2812B(0 - слева->направо | 1 - справа->налево)(только для светодиодов WS2812B)
#define BACKL_COLORS_MODE 0       //порядок цветов светодиодов WS2812B(0 - порядок цветов GRB | 1 - порядок цветов RGB)(только для светодиодов WS2812B)


//Настройки модуля веб
#define ESP_ENABLE 0              //включить поддержку внешнего модуля ESP8266 на шине I2C(0 - модуль не используется | 1 - модуль используется)

//Настройки модуля часов
#define DS3231_ENABLE 2           //включить поддержку модуля часов DS3231(0 - модуль не используется(или используется на стороне esp) | 1 - используется только модуль часов | 2 - используется модуль часов и встроенный датчик температуры)
...
//Настройки датчика яркости освещения
...
#define SQW_PORT_ENABLE 0         //включить использование порта и синхронизации секунд по SQW(0 - порт и синхронизация не используются | 1 - порт и синхронизация используются)
...
#define DEFAULT_BASE_SOUND 1       //звук нажатия клавиш(озвучка) по умолчанию(для бузера)(0 - звук кнопок и хода секунд выключены | 1 - включить звук кнопок | 2 - включить звук кнопок и хода секунд), (для плеера)(0 - озвучка выключена | 1 - озвучка всех действий), (для обычного режима управления)(2 - озвучка только времени и температуры)
...
#define DEFAULT_BACKL_MODE 1       //режим подсветки по умолчанию(для обычных светодиодов)(0 - выкл | 1 - статичная | 2 - дыхание), (для светодиодов WS2812B)(0 - выкл | 1 - статичная | 2 - дыхание | 3 - дыхание со сменой цвета при затухании | 4 - бегущий огонь | 5 - бегущий огонь со сменой цвета | 6 - бегущий огонь с радугой | 7 - бегущий огонь с конфетти | 8 - волна | 9 - волна со сменой цвета | 10 - волна с радугой | 11 - волна с конфетти | 12 - плавная смена цвета | 13 - радуга | 14 - конфетти | 15 - свечи)
#define DEFAULT_BACKL_COLOR 0      //цвет подсветки по умолчанию(0..28)(26, 27, 28 - белый цвет)(только для светодиодов WS2812B)
#define DEFAULT_DOT_MODE 2         //режим работы секундной точки по умолчанию(0..9)(описание ниже в таблице)

//Режимы секундных точек
//SECS_DOT 0..1 - (0 - выкл | 1 - статичная | 2 - мигает один раз в секунду | 3 - мигает два раза в секунду | 4 - динамичная(плавно мигает)), (для точек в индикаторах)(5 - мигающая | 6 - бегущая | 7 - змейка | 8 - резинка | 9 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 10 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки) | 11 - маятник со смещением(только для 6-ти ламп и если используются левые и правые точки))
//SECS_DOT 2 - (0 - выкл | 1 - статичная | 2 - мигает один раз в секунду | 3 - мигает два раза в секунду | 4 - динамичная(плавно мигает) | 5 - маятник(мигает один раз в секунду) | 6 - маятник(плавно мигает)), (для точек в индикаторах)(7 - мигающая | 8 - бегущая | 9 - змейка | 10 - резинка | 11 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 12 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки) | 13 - маятник со смещением(только для 6-ти ламп и если используются левые и правые точки))
//SECS_DOT 3 - (0 - выкл | 1 - статичная | 2 - мигает один раз в секунду | 3 - мигает два раза в секунду | 4 - мигающая | 5 - бегущая | 6 - змейка | 7 - резинка | 8 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 9 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки) | 10 - маятник со смещением(только для 6-ти ламп и если используются левые и правые точки))
//SECS_DOT 4 - (0 - выкл | 1 - статичная | 2 - мигает один раз в секунду | 3 - мигает два раза в секунду | 4 - метроном | 5 - шажки | 6 - таймер | 7 - маятник | 8 - луна), (для точек в индикаторах)(9 - мигающая | 10 - бегущая | 11 - змейка | 12 - резинка | 13 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 14 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки) | 15 - маятник со смещением(только для 6-ти ламп и если используются левые и правые точки))
...
//Настройки анимаций индикаторов
...
//Настройки глюков
#define DEFAULT_GLITCH_MODE 1      //режим анимации глюков по умолчанию(0 - отключен | 1 - включен)
...
#define DEFAULT_BURN_PERIOD 30     //период отображения анимации антиотравления(10..180)(мин)
...
#define BURN_DOTS 1                //включать разделительные точки в индикаторах во время антиотравления(0 - точки отключены | 1 - точки включены)


//Настройки авто-показа
#define DEFAULT_AUTO_SHOW_TIME 0   //интервал автоматического показа температуры по умолчанию(0..15)(0 - выключить автоматический показ)(мин)
...
//Настройки показа даты
#define SHOW_DATE_TIME 3000        //время отображения даты(1000..15000)(мс)
#define SHOW_DATE_TYPE 0           //формат отображения даты(0 - ДД:ММ | 1 - ММ:ДД), (для 6-ти ламп)(2 - ДД:ММ:ГГ | 3 - ММ:ДД:ГГ)
...
#define SHOW_TEMP_MAIN_SENS 0      //основной датчик для ручного отображения(0 - датчик в часах | 1 - датчик в есп)(только для esp)
#define SHOW_TEMP_CORRECT_MODE 1   //выбор датчика для коррекции температуры(0 - без коррекции | 1 - датчик в часах | 2 - датчик в есп)(только для esp)
...
#define SHOW_TEMP_COLOR_T 85       //цвет показаний температуры(0..255)(только для светодиодов WS2812B)
#define SHOW_TEMP_COLOR_P 0        //цвет показаний давления(0..255)(только для светодиодов WS2812B)
#define SHOW_TEMP_COLOR_H 170      //цвет показаний влажности(0..255)(только для светодиодов WS2812B)
#define SHOW_TEMP_BACKL_N 253      //цвет незначащих разрядов меню температуры(0..255)(только для светодиодов WS2812B)


//Настройки звука смены часа
#define DEFAULT_HOUR_SOUND_TYPE 0  //тип озвучивания текущего часа(0 - звук при смене часа выключен | 1 - только мелодия), (для SD/DF плеера)(2 - только озвучка | 3 - мелодия и озвучка)
...
#define HOUR_SOUND_MAIN_SENS 0     //основной датчик для озвучки температуры при смене часа(0 - датчик в часах | 1 - датчик в есп)(только для esp)

//Настройки приглушения звука
#define DEFAULT_SOUND_MUTE_START 8 //время начала приглушения звуков по умолчанию(0..23)(ч)(DEFAULT_SOUND_MUTE_START = DEFAULT_SOUND_MUTE_END - выключить звук)
#define DEFAULT_SOUND_MUTE_END 8   //время окончания приглушения звуков по умолчанию(0..23)(ч)

//Настройки плеера
#define DEFAULT_PLAYER_VOLUME 7    //громкость звуков плеера по умолчанию(0..15)
#define PLAYER_VOICE_MAX 2         //количество голосов озвучки записанных на карте памяти(1..5)
#define PLAYER_ALARM_MAX 3         //количество треков будильник записанных на карте памятиа(1..99)

#define PLAYER_SPEAK_MUTE 1        //запретить озвучку времени/температуры в режиме приглушения(0 - озвучка времени/температуры разрешена в период беззвучного режима | 1 - озвучка времени/температуры запрещена в период беззвучного режима)
...
#define DEFAULT_ALARM_MODE 0       //режим будильника по умолчанию(0..4)(0 - выкл | 1 - одиночный | 2 - ежедневный | 3 - по будням | 4 - по дням недели)
...
//Режимы секундных точек
//SECS_DOT 0..1 - (0 - без реакции | 1 - выкл | 2 - статичная  | 3 - мигает один раз в секунду | 4 - мигает два раза в секунду| 5 - динамичная(плавно мигает)), (для точек в индикаторах)(6 - мигающая | 7 - бегущая | 8 - змейка | 9 - резинка | 10 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 11 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки) | 12 - маятник со смещением(только для 6-ти ламп и если используются левые и правые точки))
//SECS_DOT 2 - (0 - без реакции | 1 - выкл | 2 - статичная  | 3 - мигает один раз в секунду | 4 - мигает два раза в секунду | 5 - динамичная(плавно мигает) | 6 - маятник(мигает один раз в секунду) | 7 - маятник(плавно мигает)), (для точек в индикаторах)(8 - мигающая | 9 - бегущая | 10 - змейка | 11 - резинка | 12 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 13 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки) | 14 - маятник со смещением(только для 6-ти ламп и если используются левые и правые точки))
//SECS_DOT 3 - (0 - без реакции | 1 - выкл | 2 - статичная  | 3 - мигает один раз в секунду | 4 - мигает два раза в секунду | 5 - мигающая | 6 - бегущая | 7 - змейка | 8 - резинка | 9 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 10 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки) | 11 - маятник со смещением(только для 6-ти ламп и если используются левые и правые точки))
//SECS_DOT 4 - (0 - без реакции | 1 - выкл | 2 - статичная | 3 - мигает один раз в секунду | 4 - мигает два раза в секунду | 5 - метроном | 6 - шажки | 7 - таймер | 8 - маятник | 9 - луна), (для точек в индикаторах)(10 - мигающая | 11 - бегущая | 12 - змейка | 13 - резинка | 14 - одинарный маятник(только для 6-ти ламп или если используются левые и правые точки) | 15 - двойной маятник(только для 6-ти ламп и если используются левые и правые точки) | 16 - маятник со смещением(только для 6-ти ламп и если используются левые и правые точки))
