...
#define BACKL_TYPE 2              //тип родсветки индикаторов(0 - хардверный шим(обычные светодиоды)(только D11)(только без SD плеера и хардверным UART) | 1 - софтверный шим(обычные светодиоды)(любой пин)(только без IR приемника) | 2 - светодиоды WS2812B(любой пин))
...
#define SENS_BME_ENABLE 0         //включить поддержку датчиков температуры BMPxxx и BMExxx(0 - датчик не используется | 1 - датчик используется)
...
#define DEFAULT_SECONDS_ANIM 1     //режим постоянной анимации секунд(только для 6-ти ламп)
#define SECONDS_ANIM_1_TIME 50     //скорость шага эффекта анимации секунд 1(только для 6-ти ламп)(50..80)(мс)
#define SECONDS_ANIM_2_TIME 500    //период смены яркости эффекта анимации секунд 2(только для 6-ти ламп)(50..80)(мс)
...
#define FLIP_MODE_2_TIME 500      //период смены яркости эффекта номер 2(500..1500)(мс)
...
#define BACKL_COLOR_1_MENU 255     //цвет активных разрядов меню(0..255)
#define BACKL_COLOR_2_MENU 0       //цвет неактивного разрядов меню(0..255)

#define BACKL_COLOR_TEMP 170       //цвет меню температуры(0..255)
#define BACKL_COLOR_PRESS 0        //цвет меню давления(0..255)
#define BACKL_COLOR_HUM 85         //цвет меню влажности(0..255)
...
#define GLITCH_PHASE_MIN 10        //смещение фазы периода отображения глюков(5..50)(с)
#define GLITCH_PHASE_MAX 50        //смещение фазы периода отображения глюков(5..50)(с)
...
#define AUTO_TEMP_PHASE_MIN 10     //смещение фазы периода автоматического показа температуры(5..50)(с)
#define AUTO_TEMP_PHASE_MAX 50     //смещение фазы периода автоматического показа температуры(5..50)(с)
