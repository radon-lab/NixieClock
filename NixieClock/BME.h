...
#define BME_CHECK_TIMEOUT 120 //таймаут ожидания(мс)
...
boolean readCalibration(void) //чтение калибровок датчика
{
  if (WireRequestFrom(BME_ADDR, 0x88)) return 1; //запрашиваем чтение данных, если нет ответа выходим
  ...
  if (WireRequestFrom(BME_ADDR, 0xE1)) return 1; //запрашиваем чтение данных, если нет ответа выходим
...
{
  if (!sens.initBME) { //если датчик не настроен
    if (readCalibration()) { //читаем калибровки датчика
    ...
    sens.initBME = 1; //устанавливаем флаг инициализации датчика
  }
