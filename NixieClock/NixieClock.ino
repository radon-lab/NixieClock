/*
  Arduino IDE 1.8.13 версия прошивки 1.4.3 релиз от 24.09.21
...
void timerStopwatch(void) //таймер-секундомер
{
  ...
  uint16_t tmrTime = 60; //время таймера сек
  uint16_t cntMillis = 0; //счетчик миллисекун
  uint16_t cntSec = 0; //счетчик секунд
  uint16_t msTmr = 10000UL / mainSettings.timePeriod * 2;
    ...
    if (!_timer_ms[TMR_MS]) {
      ...
      if (tmrActive && cntSec < 3600) {
        ...
        if (cntSec) indiPrintNum((mode) ? (100 - cntMillis / 10) : (cntMillis / 10), 4, 2, 0); //вывод милиекунд
          ...
          blink_data = 0; //сбрасываем мигание индикаторов
          dotState = 0; //выключаем точки
          tmrActive = 0; //деактивируем таймер
          cntMillis = 0; //сбрасываем счетчик миллисекунд
          cntSec = tmrTime; //сбрасываем таймер
        }
      }
    }
