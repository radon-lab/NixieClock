...
#define SHT20_READ_TEMP 0xF3
#define SHT20_READ_HUM 0xF5

#define SHT20_TEMP_TIME 15
#define SHT20_HUM_TIME 20

#define SHT30_ADDR 0x44 //адрес датчика

#define SHT30_MEAS_TIME 20

#define SHT30_READ_DATA 0x24
#define SHT30_RESOLUTION 0x00
...
void readTempSHT(void) //чтение температуры/влажности
{
  ...
  switch (typeSHT) { //чтение данных
    case SHT20_ADDR: {
        ...
        sens.temp = temp_raw * 0.0268 - 468; //рассчитываем температуру
        ...
        wireReadEndByte(); //пропускаем контрольную сумму
        sens.hum = hum_raw * 0.0019 - 6; //рассчитываем влажность
      }
      break;
    case SHT30_ADDR: {
        ...
        wireReadEndByte(); //пропускаем контрольную сумму
        sens.temp = temp_raw * 0.0267 - 450;  //рассчитываем температуру
