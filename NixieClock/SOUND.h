...
void buzzPulse(uint16_t freq, uint16_t time) //генерация частоты бузера (частота 10..10000, длительность мс.)
{
  ...
  if (freq) { //если частота установлена
...
//---------------------------Состояние воспроизведения мелодии-------------------------------------------
inline boolean melodyState(void) //состояние воспроизведения мелодии
{
  return (boolean)(sound.replay != REPLAY_STOP);
}
