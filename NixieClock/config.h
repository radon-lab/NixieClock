#define BOARD_TYPE 0            //тип платы часов(0 - IN-12 (индикаторы стоят правильно) | 1 - IN-12 turned (индикаторы перевёрнуты) | 2 - IN-14 (обычная и neon dot) | 3 - другие индикаторы)
#define LAMP_NUM 4              //количесвто ламп(4 или 6)(экспериментальная функция)
...
#define HIGH_PULL               //тип кнопок(HIGH_PULL - кнопки подтянуты к питанию "+" | LOW_PULL - кнопки подтянуты к земле "-" | ANALOG - используятся аналоговые кнопки)
...
#define FLIP_MODE_5_TIME 100    //скорость шага эффекта номер 5(50..170)(мс)
...
#define DIMM_FREQ 4                  //частота динамической индикации(4 - 60Гц | 3 - 240Гц | 2 - 1900Гц)

#define DEFAULT_TIME_PERIOD 2048     //период тика таймера счета времени(мкс)
#define TIME_PERIOD_MIN 1800         //минимальный период тика таймера счета времени(мкс)
#define TIME_PERIOD_MAX 2200         //максимальный период тика таймера счета времени(мкс)
...
#define EEPROM_BLOCK_CRC 255         //блок памяти версии прошивки
