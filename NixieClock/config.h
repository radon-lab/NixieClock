...
#define DECODER_MASK ID(0), ID(1), ID(2), ID(3), ID(4), ID(5), ID(6), ID(7), ID(8), ID(9), ID(10) //порядок соответствия выходов дешифратора катодам в лампе(указывать через запятую, от цифры 0 до цифры 9)(ID(10) должен быть всегда в конце)(ID(0)..ID(9))
...
//Символьная лампа
#define SYMB_POSITIVE ID(10) //соответствие выхода дешифратора символу "плюс"(ID(0)..ID(9)(ID(10) - не используется)
#define SYMB_NEGATIVE ID(10) //соответствие выхода дешифратора символу "минус"(ID(0)..ID(9)(ID(10) - не используется)
#define SYMB_PRESSURE ID(10) //соответствие выхода дешифратора символу "давление"(ID(0)..ID(9)(ID(10) - не используется)
#define SYMB_HUMIDITY ID(10) //соответствие выхода дешифратора символу "влажность"(ID(0)..ID(9)(ID(10) - не используется)
#define SYMB_ALARM ID(10) //соответствие выхода дешифратора символу "будильник"(ID(0)..ID(9)(ID(10) - не используется)
#define SYMB_RADIO ID(10) //соответствие выхода дешифратора символу "радиостанция"(ID(0)..ID(9)(ID(10) - не используется)
#define SYMB_MENU ID(10) //соответствие выхода дешифратора символу "меню"(ID(0)..ID(9)(ID(10) - не используется)
...
//Настройки анимаций перелистования цифр
...
//Настройки анимаций смены секунд
...
//Настройки анимаций секундных точек
...
#define DOT_SHIFT_TURN_TIME 500      //время переключения точек в индикаторах для анимации "маятник со смещением"(100..500)(0 - один раз в секунду)(мс)(только для разделительных точек)
...
#define BACKL_MODE_15_TIME 20        //скорость шага эффекта подсветки "свечи"(10..100)(мс)(только для светодиодов WS2812B)
#define BACKL_MODE_15_MOVE_TIME 100  //скорость изменения направления эффекта подсветки "свечи"(50..300)(мс)(только для светодиодов WS2812B)
#define BACKL_MODE_15_SMOOTH 15      //коэффициент плавности эффекта "свечи"(0..255)
#define BACKL_MODE_15_COLOR 10       //начальный цвет эффекта "свечи"(чем больше - тем горячее(белее))(0..100)
#define BACKL_MODE_15_CAST 10        //уровень заброса цвета эффекта "свечи"(чем больше - тем дальше заброс по цвету)(0..50)
...
#define TEST_START_MODE 0            //режим запуска тестирования системы(0 - запускается по нажатию кнопки | 1 - запускается при подаче питания)
...
#define TEMP_UPDATE_TIME 60000       //частота обновления показаний датчика температуры(2000..60000)(мс)
...
#define SQW_TEST_TIME 2000           //длительность проверки наличия сигнала с пина SQW при запуске(1500..5000)(мс)
#define SQW_MIN_TIME 650             //минимальная длительность сигнала с пина SQW во время работы(500..1000)(мс)
#define SQW_MAX_TIME 2000            //максимальная длительность сигнала с пина SQW во время работы(1500..3000)(мс)
...
#define GEN_SPEED_X2 0               //повысить частоту шим ВВ преобразователя в 2 раза(0 - частота 31250 Гц | 1 - частота 62500 Гц)

//Настройки динамической индикации
#define INDI_FREQ_ADG 90             //частота динамической индикации(50..150)(гц)
#define INDI_DEAD_TIME 30            //длительность паузы после закрытия оптопар(0..50)
#define INDI_MIN_BRIGHT 5            //минимальная яркость индикаторов доступная для настройки(5..10)

//Настройки напряжения
#define REFERENCE 110                //опорное напряжение(100..120)(мв/10)
#define MAX_VCC 550                 //максимальное напряжение питания(400..550)(мв/10)
#define MIN_VCC 450                  //минимальное напряжение питания(400..550)(мв/10)


//Настройки светодиодов подсветки
#define LEDS_NUM (LAMP_NUM)          //количество светодиодов подсветки(LAMP_NUM..255)(только для светодиодов WS2812B)

#define LED_DIGIT_H1 0               //номер светодиода для первого разряда часа(0..255)(только для светодиодов WS2812B)
#define LED_DIGIT_H2 1               //номер светодиода для второго разряда часа(0..255)(только для светодиодов WS2812B)
#define LED_DIGIT_M1 2               //номер светодиода для первого разряда минут(0..255)(только для светодиодов WS2812B)
#define LED_DIGIT_M2 3               //номер светодиода для второго разряда минут(0..255)(только для светодиодов WS2812B)
#define LED_DIGIT_S1 4               //номер светодиода для первого разряда секунд(0..255)(только для светодиодов WS2812B)
#define LED_DIGIT_S2 5               //номер светодиода для второго разряда секунд(0..255)(только для светодиодов WS2812B)
...
#define LIGHT_SENS_START_MIN 80      //стартовое значение минимального диапазона(0..255)
#define LIGHT_SENS_START_MAX 180     //стартовое значение максимального диапазона(0..255)
...
#define IR_EXT_BTN_ENABLE 1          //разрешить использование расширеных кнопок пульта(0 - запрещено(8 кнопок) | 1 - разрешено(13 кнопок) | 2 - разрешено(23 кнопок))

//Настройки кнопок
#define BTN_GIST_TIME 50             //время для защиты от дребезга кнопки(0..250)(мс)
#define BTN_HOLD_TIME 550            //время длительного удержания кнопки(200..1500)(мс)

#define BTN_ANALOG_GIST 25           //гистерезис между значениями ацп кнопок(5..50)
...
//Настройки звука хода секунд
#define SECS_EVEN_SOUND_FREQ 100    //частота звука хода четных секунд(10..10000)(Гц)
#define SECS_EVEN_SOUND_TIME 10      //длительность звука хода четных секунд(10..500)(мс)
#define SECS_UNEVEN_SOUND_FREQ 180  //частота звука хода нечетных секунд(10..10000)(Гц)
#define SECS_UNEVEN_SOUND_TIME 7     //длительность звука хода нечетных секунд(10..500)(мс)
...
//Настройки частот радио
#define RADIO_MIN_FREQ 870           //минимальная частота радио(870)
#define RADIO_MAX_FREQ 1080          //максимальная частота радио(1080)

//Настройки общей громкости
#define MAIN_MIN_VOL 0               //общая минимальная громкость(0)
#define MAIN_MAX_VOL 15              //общая максимальная громкость(15)
...
#define EEPROM_BLOCK_INFO 1001       //блок памяти информации о плате(1001)
#define EEPROM_BLOCK_BOOT 1023       //блок памяти состояния загрузчика(1023)
