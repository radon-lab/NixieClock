...
#define HV_VCC 170              //напряжение питания ламп(150..190)(в)
#define HV_R_HIGH 680           //сопротивление верхнего плеча резисторного делителя(кОм)
#define HV_R_LOW 10             //сопротивление нижнего плеча резисторного делителя(кОм)

#define FREQ_ADG 70             //частота динамической индикации(4 лампы - (62..122) | 6 ламп - (40..80))(гц)

#define MIN_PWM 100             //минимальная скважность(индикаторы выключены, яркость минимальная)(100..150)
#define MAX_PWM 180             //максимальная скважность(индикаторы включены, яркость максимальная)(150..200)
...
#define DEFAULT_FLIP_ANIM 1     //режим перелистования цифр по умолчанию(0 - выкл | 1 - рандомная смена эффектов | 2 - плавное угасание и появление | 3 - перемотка по порядку числа | 4 - перемотка по порядку катодов в лампе | 5 - поезд | 6 - резинка | 7 - волна | 8 - блики | 9 - испарение)
...
#define DEFAULT_BACKL_BRIGHT 250     //яркость подсветки ламп дневная(10..250)(шаг 10)
#define DEFAULT_BACKL_BRIGHT_N 120   //яркость подсветки ламп ночная(0..250)(0 - подсветка выключена)(шаг 10)
#define BACKL_MIN_BRIGHT 30          //минимальная яркость подсветки ламп(0..120)
...
#define BACKL_MODE_2_TIME 5000       //период эффекта подсветки номер 2(100..12000)(мс)
#define BACKL_MODE_3_COLOR 25        //шаг изменения цвета эффекта подсветки номер 3(5..50)

#define BACKL_MODE_4_TIME 25         //скорость шага эффекта подсветки номер 4(4..100)(мс)
#define BACKL_MODE_4_FADING 10       //количество шагов декркминента яркости эффекта подсветки номер 4(5..50)
#define BACKL_MODE_4_STEP 10         //шаг декркминента яркости эффекта подсветки номер 4(1..50)
#define BACKL_MODE_5_COLOR 5         //шаг изменения цвета эффекта подсветки номер 5(5..50)

#define BACKL_MODE_6_TIME 5000       //период эффекта подсветки номер 6(100..12000)(мс)
#define BACKL_MODE_6_STEP 1          //максимальный размер шага яркости эффекта подсветки номер 6(1..50)
...
#define REFERENCE 1.1                //опорное напряжение(1.0..1.2)
...
#define BTN_RIGHT_R_HIGH 0           //сопротивление резистора "ПРАВОЙ" клавиши(кОм)(0..255)
